...
<div class="mb-4">
    ...
    <h1 class="h2 font-heading mt-2" style="color: var(--text-primary);">Ubah Kategori Barang</h1>
    <span class="text-muted small">Ubah informasi klasifikasi barang</span>
</div>
...
<div class="row">
    <div class="col-md-8">
        <div class="glass-card p-4">
            ...
            <form action="index.php?controller=kategori&action=edit&id=<?= $category['kategori_id'] ?>" method="POST" autocomplete="off">
                <!-- CSRF Token -->
                <input type="hidden" name="csrf_token" value="<?= Session::getCsrfToken() ?>">

                <div class="mb-3">
                    <label for="nama_kategori" class="form-label text-muted small">Nama Kategori <span class="text-danger">*</span></label>
                    <input type="text" class="form-control <?= isset($errors['nama_kategori']) ? 'is-invalid' : '' ?>" id="nama_kategori" name="nama_kategori" value="<?= htmlspecialchars($_POST['nama_kategori'] ?? $category['nama_kategori']) ?>" required>
                ...
                <div class="mb-4">
                    <div class="form-check form-switch bg-tertiary p-3 rounded-3 border" style="border-color: var(--border-color); padding-left: 2.85rem;">
                        <input class="form-check-input" type="checkbox" role="switch" id="is_critical" name="is_critical" value="1" <?= (($_POST['is_critical'] ?? $category['is_critical']) == '1') ? 'checked' : '' ?>>
